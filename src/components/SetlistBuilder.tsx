...
export default function SetlistBuilder({ schedule }: { schedule: Schedule }) {
  ...
  const [draggingSetlistId, setDraggingSetlistId] = useState<string | null>(null);
  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    const internalDragId = e.dataTransfer.getData('application/internal-setlist-id');
    if (internalDragId && draggingSetlistId) {
      // Find the item being dropped on by figuring out drop target?
      // Actually we'll implement a separate handleRowDrop for internal.
      return; 
    }
  ...
  const handleRowDragStart = (e: React.DragEvent, id: string) => {
    e.stopPropagation();
    setDraggingSetlistId(id);
    e.dataTransfer.setData('application/internal-setlist-id', id);
  };
  ...
  const handleRowDrop = async (e: React.DragEvent, targetIndex: number) => {
    ...
    // First figure out the new sort order
    let newSortOrder = 0;
    const isInternal = !!draggingSetlistId;
    const sourceIndex = isInternal ? setlist.findIndex(s => s.id === draggingSetlistId) : -1;

    if (isInternal && (sourceIndex === -1 || sourceIndex === targetIndex)) return;

    if (targetIndex === 0) {
      newSortOrder = setlist[0].sort_order - 10;
    } else if (isInternal && targetIndex >= setlist.length - 1 && sourceIndex < targetIndex) {
      newSortOrder = setlist[setlist.length - 1].sort_order + 10;
    } else {
       const prevOrder = (isInternal && targetIndex < sourceIndex) ? setlist[targetIndex - 1]?.sort_order ?? 0 : setlist[targetIndex].sort_order;
       const nextOrder = (isInternal && targetIndex < sourceIndex) ? setlist[targetIndex].sort_order : setlist[targetIndex + 1]?.sort_order ?? (setlist[targetIndex].sort_order + 10);
       newSortOrder = (prevOrder + nextOrder) / 2;
    }

    if (isInternal) {
      try {
        await updateDoc(doc(db, 'setlists', draggingSetlistId as string), { sort_order: newSortOrder });
      } catch (err) {
        handleFirestoreError(err, OperationType.UPDATE, `setlists/${draggingSetlistId}`);
      }
      setDraggingSetlistId(null);
    } else {
      // External drag
      setIsDragOver(false);
      try {
