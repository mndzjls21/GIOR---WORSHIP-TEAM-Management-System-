    ...
    <div className="flex-1 overflow-y-auto bg-slate-50 dark:bg-zinc-950 transition-colors p-4 md:p-8 min-h-0 container mx-auto">
      <div className="mb-6 flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500 dark:text-zinc-400 text-sm">View scheduling for worship team members across all upcoming services.</p>
        </div>
      </div>

      <div className="flex flex-col gap-6">
        {sortedSchedules.length === 0 ? (
          <div className="text-center py-12 text-slate-500 dark:text-zinc-400 italic">No services scheduled yet.</div>
        ) : (
            ...
            <div key={schedule.id} className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-white/10 transition-colors rounded-lg p-6 relative overflow-hidden group shadow-sm dark:shadow-none">
              ...
              <div className="flex flex-col md:flex-row gap-8">
                <div className="w-full md:w-1/4 shrink-0 border-b md:border-b-0 md:border-r border-slate-200 dark:border-white/10 transition-colors pb-4 md:pb-0 md:pr-4">
                  ...
                  <div className="flex items-center gap-2 mt-4">
                    <button 
                      ...
                      className="flex-1 flex justify-center items-center gap-1.5 py-1.5 text-[10px] font-bold uppercase tracking-widest text-slate-500 hover:text-slate-900 dark:hover:text-slate-100 border border-slate-200 hover:border-slate-300 dark:border-white/5 dark:hover:border-white/20 rounded transition-colors"
                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-4 w-full">
                  <RosterItem label="Presider" value={schedule.presider} />
...
function RosterItem({ label, value }: { label: string, value: string | undefined }) {
  return (
    <div className="flex flex-col">
      <span className="text-[10px] uppercase tracking-widest text-slate-500 dark:text-zinc-400 mb-1">{label}</span>
      <span className="text-sm font-medium text-slate-500 dark:text-zinc-400">
        {value && value.trim() ? value : <span className="text-slate-500 dark:text-zinc-400 italic">Unassigned</span>}
      </span>
    </div>
  )
}
