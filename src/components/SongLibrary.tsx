    ...
    <>
      <div className="p-6 border-b border-slate-300 dark:border-white/5 transition-colors flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <h3 className="text-[10px] uppercase tracking-[0.2em] text-slate-500 dark:text-zinc-400">Song Repository</h3>
          <div className="flex items-center gap-2">
            <select 
              value={groupBy}
              onChange={(e) => setGroupBy(e.target.value as any)}
              className="text-[10px] bg-transparent border border-slate-300 dark:border-white/10 text-slate-500 dark:text-zinc-400 px-2 py-1.5 rounded outline-none focus:border-slate-400 dark:focus:border-slate-500 cursor-pointer"
            >
            ...
            <button 
              ...
              className="text-[10px] text-slate-500 dark:text-zinc-400 hover:text-slate-900 dark:text-slate-100 transition-colors uppercase tracking-widest border border-slate-300 dark:border-white/10 transition-colors px-2 py-1.5 rounded flex items-center"
              title="Add New Song"
            >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-20 scrollbar-thin scrollbar-thumb-white/10 hover:scrollbar-thumb-white/20">
        {Object.entries(groupedSongs).map(([groupName, groupSongs]) => (
          <div key={groupName} className="space-y-1">
            {groupBy !== 'none' && (
              <h4 className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500 pl-3 mb-2 mt-4 first:mt-0 sticky top-0 bg-white/90 dark:bg-[#121212]/90 backdrop-blur-md py-1 z-10 border-b border-transparent dark:border-white/5 shadow-[0_4px_10px_rgba(0,0,0,0.4)] md:shadow-none font-sans">
                {groupName}
              </h4>
            )}
            {groupSongs.map(song => (
              ...
              >
                  ...
                  <div className="pr-12">
                     <h3 className="font-medium text-slate-900 dark:text-white text-sm group-hover:text-slate-900 dark:hover:text-slate-100 transition-colors cursor-pointer flex items-center gap-2">
                       ...
                       {song.language && song.language !== 'English' && (
                         <span className="text-[8px] bg-slate-200 dark:bg-white/10 text-slate-600 dark:text-slate-400 px-1 py-0.5 rounded uppercase tracking-widest">{song.language}</span>
                       )}
                     </h3>
                     <p className="text-[10px] text-slate-500 dark:text-zinc-400 mt-0.5 uppercase tracking-wider">{song.artist}</p>
                  </div>
                  <span className="font-mono text-[9px] px-1.5 py-0.5 border border-slate-300 dark:border-white/10 transition-colors text-slate-500 dark:text-zinc-400 rounded mt-0.5 whitespace-nowrap">
                ...
                {song.tags && song.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {song.tags.map(t => (
                      <span key={t} className="text-[8px] uppercase tracking-widest px-1 py-0.5 text-slate-500 dark:text-zinc-400 border border-slate-300 dark:border-white/5 transition-colors rounded-sm">
                        {t}
                      </span>
                    ))}
                  </div>
                ...
                <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    ...
                    className="p-1.5 text-slate-500 dark:text-zinc-400 hover:text-red-400 rounded-md bg-white dark:bg-zinc-900 transition-colors"
                    title="Delete"
                  >
        ...
        {filtered.length === 0 && (
          <div className="text-center p-6 text-slate-500 dark:text-zinc-400 text-xs italic">
            No songs found.
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col gap-6 font-sans">
        <div className="flex justify-between items-start border-b border-slate-300 dark:border-white/10 transition-colors pb-4">
          <div>
            ...
            <div className="flex gap-4 text-xs text-slate-500 dark:text-zinc-400 uppercase tracking-widest">
            ...
            {song?.tags && song.tags.length > 0 && (
              <div className="flex gap-2 mt-4">
                {song.tags.map(tag => (
                  <span key={tag} className="text-[10px] bg-slate-200 dark:bg-white/5 transition-colors border border-slate-300 dark:border-white/10 transition-colors rounded px-2 py-1 text-slate-500 dark:text-zinc-400 uppercase tracking-widest">{tag}</span>
                ))}
              </div>
            )}
          </div>
          <button onClick={() => setIsEditing(true)} className="flex items-center gap-2 border border-slate-300 dark:border-white/10 transition-colors px-3 py-1.5 rounded text-xs uppercase text-slate-500 dark:text-zinc-400 hover:text-slate-900 dark:text-white hover:bg-slate-200 dark:bg-white/5 transition-colors">
    ...
    <div className="fixed inset-0 bg-slate-900/40 dark:bg-black/80 transition-colors backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className={cn("bg-white dark:bg-zinc-900 transition-colors border border-slate-300 dark:border-white/10 transition-colors rounded-lg w-full max-h-[90vh] shadow-2xl flex flex-col font-sans", !isEditing && hasAnyVideo ? "max-w-5xl" : "max-w-2xl")}>
        <div className="p-4 border-b border-slate-300 dark:border-white/10 transition-colors flex justify-between items-center">
          ...
          <button onClick={onClose} className="text-slate-500 dark:text-zinc-400 hover:text-slate-900 dark:text-white">✕</button>
        </div>
        ...
        {isEditing ? (
          <form onSubmit={handleSave} className="p-6 overflow-y-auto flex-1 space-y-6 text-left">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <label className="block text-[10px] uppercase tracking-widest text-slate-500 dark:text-zinc-400 mb-2">Title *</label>
              ...
              <div>
                <label className="block text-[10px] uppercase tracking-widest text-slate-500 dark:text-zinc-400 mb-2">Artist *</label>
              ...
              <div>
                <label className="block text-[10px] uppercase tracking-widest text-slate-500 dark:text-zinc-400 mb-2">Original Key *</label>
              ...
              <div>
                <label className="block text-[10px] uppercase tracking-widest text-slate-500 dark:text-zinc-400 mb-2">BPM *</label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <label className="block text-[10px] uppercase tracking-widest text-slate-500 dark:text-zinc-400 mb-2">Language</label>
              ...
              <div>
                <label className="block text-[10px] uppercase tracking-widest text-slate-500 dark:text-zinc-400 mb-2">Tags (comma separated)</label>
            ...
            <div>
              <label className="block text-[10px] uppercase tracking-widest text-slate-500 dark:text-zinc-400 mb-2">YouTube / Media URL</label>
            ...
            <div>
              <label className="block text-[10px] uppercase tracking-widest text-slate-500 dark:text-zinc-400 mb-2">Instrumental Guide (Guitar) URL</label>
            ...
            <div>
              <label className="block text-[10px] uppercase tracking-widest text-slate-500 dark:text-zinc-400 mb-2">Instrumental Guide (Piano) URL</label>
            ...
            <div>
              <label className="block text-[10px] uppercase tracking-widest text-slate-500 dark:text-zinc-400 mb-2">Lyrics & Chords * (Use [brackets] for chords)</label>
              <p className="text-[10px] text-slate-500 dark:text-zinc-400 mb-2 italic">Example: [G]Amazing grace how [C]sweet the [G]sound</p>
            ...
            <div className="pt-4 flex justify-end gap-3">
              {song && (
                <button type="button" onClick={() => setIsEditing(false)} className="px-6 py-2 rounded text-xs uppercase tracking-widest font-bold text-slate-500 dark:text-zinc-400 hover:text-slate-900 dark:text-white transition-colors">
                  Cancel
                </button>
              )}
